...
const arduinoDisplay = document.getElementById("arduinoValue");
...
async function fetchArduinoData() {
    try {
        ...
        if (arduinoDisplay) {
            arduinoDisplay.textContent = data.value;
        }
    } catch (err) {
        console.error("Error leyendo del Arduino:", err);
    }
}

// Ejecutar cada 500 milisegundos
setInterval(fetchArduinoData, 500);
